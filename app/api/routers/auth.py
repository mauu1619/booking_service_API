...
from fastapi import Depends, Cookie, APIRouter, status, HTTPException, Response, Path
from fastapi.security import OAuth2PasswordRequestForm
from redis.asyncio import Redis

from app.core.security import AccessToken, create_token, verify_password, decode_token, get_password_hash
from app.core.config import settings
from app.models import Users, UserCreate, UserShow, UserRole
from app.db.session import get_session
from app.db.redis import get_redis_session
from app.db.crud import create_user, get_user_by_email, get_user_by_username, make_user_inactive
from app.api.deps import get_default_user, get_admin_user
...
async def login(
    ...
    logger.info("Request", method="POST", path="/auth/login", username=form_data.username)
    ...
    await redis.set(f"user:{user.id}:refresh", value=refresh_token, ex=expire_days_seconds)
...
async def refresh(
    ...
    await redis.set(f"user:{user_id}:refresh", value=refresh_token, ex=expire_days_seconds)
