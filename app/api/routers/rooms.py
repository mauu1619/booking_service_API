...
from app.models import PaginatedRooms, RoomCreate, RoomsShow, RoomUpdate, Users
...
async def rooms_list(
    ...
    return PaginatedRooms(
        items=[RoomsShow.model_validate(room) for room in rooms],
        total=total,
        limit=limit,
        offset=offset,
    )
...
async def room_detail(room_id: Annotated[int, Path(ge=1)], db: SessionDep):
...
async def room_create(room_in: RoomCreate, db: SessionDep, _: AdminDep):
...
async def room_remove(room_id: Annotated[int, Path(ge=1)], db: SessionDep, _: AdminDep):
