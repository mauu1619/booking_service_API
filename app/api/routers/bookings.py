...
async def my_bookings(user: UserDep, db: SessionDep):
...
async def cancel_booking(
    booking_id: Annotated[int, Path(ge=1)], db: SessionDep, user: UserDep
):
    logger.info(
        "Request", method="DELETE", path=f"/bookings/{booking_id}", user_id=user.id
    )
    try:
